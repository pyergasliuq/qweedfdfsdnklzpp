...
OWNER_ID = 2080411409  # creator — always developer + lifetime sub
FREE_TRIAL_STARS = 1
FREE_TRIAL_DAYS = 3
...
async def run_with_timeout(coro, timeout_sec: float, user_id: int,
    ...
    """Wraps a coroutine with a timeout for free users."""
...
def kb_admin_main(user_id: int = 0):
    ...
    b.button(text="📋 Команды", callback_data="adm_commands_list")
    b.adjust(2)
    return b.as_markup()
...
def initialize_database():
    ...
    for col4, def4 in [("trial_used","'False'"),("active_promo","NULL"),("promo_expires","NULL"),
        ...
        try:
            ...
            pass
    # Auto-set owner as developer
    c.execute("SELECT chat_id FROM users WHERE chat_id=?", (OWNER_ID,))
    if not c.fetchone():
        c.execute("INSERT INTO users (chat_id, username, sub, admin, time, role) VALUES (?,?,'True','True','31.12.2099','developer')",
                  (OWNER_ID, 'owner'))
    else:
...
def update(chat_id, username):
    ...
    if user:
        ...
        if db_expiration_date_str and chat_id != OWNER_ID:
...
async def on_payment(message: types.Message):
    ...
    # Ref reward only for real paid subs
    if not is_trial and stars > FREE_TRIAL_STARS:
        ref_result = mark_referral_paid(user_id, stars)
        if ref_result and ref_result[0]:
            r_id, r_reward, l2_id, l2_reward = ref_result
            try:
                await bot.send_message(r_id,
                    f"🌟 <b>Реферальный бонус!</b>\n"
                    f"Ваш реферал купил Premium — начислено <b>{r_reward}⭐</b>",
                    parse_mode="HTML")
            except Exception: pass
            if l2_id and l2_reward:
                try:
                    await bot.send_message(l2_id,
        ...
        if rrow:
            awarded_m, cnt_m = check_free_ref_milestone(rrow[0])
            if awarded_m:
                try:
                    await bot.send_message(rrow[0],
                        f"🎁 <b>Поздравляем!</b>\n"
                        f"Вы привели <b>{cnt_m}</b> пользователей — "
                        f"вам выдана Pro-подписка на <b>1 месяц</b>!",
                        parse_mode="HTML")
                except Exception: pass
    ...
    try:
        ...
        uname_n = message.from_user.username or ""
        name_n = f"@{uname_n}" if uname_n else f"ID:{user_id}"
        trial_tag = " [ПРОБНЫЙ]" if is_trial else ""
        notif = (
            f"💰 <b>Новая покупка!{trial_tag}</b>\n"
            f"👤 {name_n} (<code>{user_id}</code>)\n"
            f"📦 Тариф: <b>{plan_label}</b>\n"
            f"⭐ Звёзд: <b>{stars}</b> | До: <b>{expiry}</b>"
        )
        for adm_id in adm_ids:
            if adm_id != user_id:
                try: await bot.send_message(adm_id, notif, parse_mode="HTML")
                except Exception: pass
    except Exception: pass
...
@dp.callback_query(F.data == "adm_commands_list")
async def cb_adm_commands_list(callback: types.CallbackQuery):
    ...
    text = (
        ...
        "<code>/sub ID True ДД.ММ.ГГГГ</code> — выдать подписку\n"
        "<code>/sub ID False</code> — забрать подписку\n"
        "<code>/send ID текст</code> — сообщение пользователю\n\n"
        "<b>📢 Рассылка:</b>\n"
        "<code>/kotek текст</code> — рассылка всем\n\n"
        "<b>📣 Каналы подписки:</b>\n"
        "<code>/addchannel @username</code> — добавить канал\n"
        "<code>/delchannel @username</code> — удалить канал\n\n"
        "<b>👑 Роли:</b>\n"
        "<code>ID роль</code> — через меню Управление ролями\n"
        "Роли: <code>moderator</code>, <code>admin</code>, <code>developer</code>\n\n"
        "<b>🎟 Промокоды:</b>\n"
        "Создание/деактивация/замена — через меню Промокоды\n\n"
        "<b>🎫 Тикеты:</b>\n"
        "Просмотр и ответ — через меню Тех. поддержка\n\n"
        "<b>🎁 Выдать подписку вручную:</b>\n"
        "Через меню → Выдать подписку"
    )
    b = InlineKeyboardBuilder()
    b.button(text="⬅️ Назад", callback_data="adm_main")
...
def _period_kb(type_key: str):
    b = InlineKeyboardBuilder()
    for lbl, p in [("1ч","hour"),("24ч","day"),("7д","week"),("1м","month"),("3м","quarter"),("1г","year")]:
...
async def cb_adm_chart_type(callback: types.CallbackQuery):
    ...
    names = {"reg":"👥 Регистрации","purchases":"💰 Покупки","refs":"👥 Рефералы",
             "promos":"🎟 Промокоды","tickets":"🎫 Тикеты"}
    await callback.message.edit_text(
...
async def cb_adm_promo_replace(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "<code>НОВЫЙ_КОД</code>\n\n"
        "Старый деактивируется, у всех пользователей активируется новый.",
        parse_mode="HTML")
    await callback.answer()
...
async def fsm_promo_code_input(message: types.Message, state: FSMContext):
    ...
    if action == "deact":
        ...
        old_p, new_p = get_promo(old_code), get_promo(new_code)
        if not old_p:
            await message.answer(f"❌ Промокод <code>{old_code}</code> не найден.", parse_mode="HTML"); return
        if not new_p:
        ...
        affected = c_rp.rowcount; conn_rp.commit(); conn_rp.close()
        await message.answer(
...
async def _doc_inner(message: types.Message, state: FSMContext):
    ...
    if current_state == BatchFSM.collecting.state:
        added = add_batch_file(user_id, message.document.file_id, message.document.file_name)
        if added:
            batch_now = get_batch(user_id)
            cnt_b = len(batch_now.get("files", []))
            await message.answer(
                f"📎 Файл <b>{cnt_b}</b> добавлен: <code>{message.document.file_name}</code>\n"
...
async def _ok_inner(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    username = message.from_user.username or f"user_{user_id}"
    ...
    try:
        ...
        if "/timecyc" in message.text and len(j) >= 5:
            ...
            await message.answer(
                "<b>Привет👋 Вот возможности бота:</b>\n\n"
