...
PRESETS = {
    "1": {"name": "⚡ Ускор + Антик", "folder": "preset1", "desc": "Ускоренная стрельба, антикилл, раскрывающийся прицел"},
    "2": {"name": "🔄 Без перезарядки + Динамичный", "folder": "preset2", "desc": "Без перезарядки, динамичный прицел"},
    "3": {"name": "🎯 Без перезарядки + Статичный", "folder": "preset3", "desc": "Без перезарядки, статичный прицел"},
}
...
async def handle_valid_files(message: types.Message, ):
    ...
    progress_msg = await message.reply(f"<b>⏳ Обрабатываю ваш файл...</b>", parse_mode="HTML", force_document=True)
