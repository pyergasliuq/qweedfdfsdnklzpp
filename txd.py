...
class TXDConverter:
    ...
    def decode_bgr888(self, data, width, height):
        try:
            ...
            logging.error(f"Ошибка декодирования BGR888: {e}")
...
async def txd(message: types.Message, bot: Bot):
    ...
    msg = await message.answer(
        f"<b>Чтобы распаковать ваш файл TXD в ZIP отправьте файл - TXD</b>"
    )
...
async def txd_xyina(message: types.Message, bot: Bot):
    ...
    try:
        ...
        await message.reply_document(
            result,
            caption=f"<b>📊 Ваш файл готов</b>\n<b>📝 Название - {os.path.splitext(message.document.file_name)[0]}.zip</b>"
        )
    except Exception as e:
        logging.error(f"TXD processing error: {e}", exc_info=True)
        await progress_msg.edit_text(f"<b>❌ Ой, {name}! Произошла ошибка: {str(e)}</b>")
    finally:
