...
def setup_converter(bot: Bot):
            ...
            if zip_path:
                try:
                    await bot.send_document(
                        ...
                        caption=f"<b>📊 Ваш файл готов</b>\n<b>📝 Название - wirhub.convert.zip</b>",
    ...
    async def process_files(message: types.Message, files: List[types.Message]) -> None:
        try:
            ...
            await message.reply(f"⚠️ Ошибка при обработке файлов")
        finally:
    ...
    async def convert_command(message: types.Message, bot: Bot):
        ...
        await message.answer(
            ...
            f"<b>🗂 Просто отправляй мне нужные файлы, а я всë сделаю сам!</b>",
            parse_mode=ParseMode.HTML
        )
    ...
    async def handle_conversion(message: types.Message, bot: Bot):
        ...
        try:
            ...
            await progress_msg.edit_text(f"<b>Произошла ошибка при конвертации</b>", parse_mode="HTML")
        finally:
